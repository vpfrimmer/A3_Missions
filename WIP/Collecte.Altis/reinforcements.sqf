...
_nul = [EAST, "spawnMrk", "LZMrk", 2, true, true, true, true, false] spawn TAG_fnc_reinforcements; <<
 ...
 ---------------------------------------------------------------------------------------------------------*/
 TAG_fnc_reinforcements_call = {
	_null = [_this] spawn TAG_fnc_reinforcements; 
 };
...
switch (_side) do {
	...
	case EAST : {
        ...
        _helo = [getMarkerPos _spawnMrk, markerDir _spawnMrk, "I_Heli_Transport_02_F", EAST] call BIS_FNC_spawnVehicle;
		_infgrp = [getMarkerPos _spawnMrk, EAST, (configFile >> "CfgGroups" >> "Indep" >> "IND_F" >> "Infantry" >> _ranGrp)] call BIS_fnc_spawnGroup;
	};
};
